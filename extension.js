...
var AppGridButton = GObject.registerClass(
  class AppGridButton extends PanelMenu.Button {
    _init() {
      ...
      this.app_grid_button.connect("button-press-event", () =>
...
var FavoritesMenu = GObject.registerClass(
  class FavoritesMenu extends PanelMenu.Button {
    _init() {
      ...
      this.fav_changed = AppFavorites.getAppFavorites().connect(
        "changed",
        this._display_favorites.bind(this)
      );

      // make menu button
      this.fav_menu_button = new St.BoxLayout({});
      this.fav_menu_icon = new St.Icon({
        icon_name: FAVORITES_ICON_NAME,
        style_class: "system-status-icon",
      });
    ...
    _display_favorites() {
      ...
      for (let fav_index = 0; fav_index < this.list_fav.length; ++fav_index) {
        ...
        this.item_box = new St.BoxLayout({
          style_class: "favorite",
          vertical: false,
        });
...
var WindowControlButton = GObject.registerClass(
  class WindowControlButton extends PanelMenu.Button {
    _init() {
      ...
      this.window_control_button = new St.BoxLayout({
        visible: true,
        reactive: true,
        can_focus: true,
        track_hover: true,
      ...
      this.window_control_button_previous = new St.Bin({
        visible: true,
        reactive: true,
        can_focus: true,
        track_hover: true,
      });
      this.window_control_button_previous_icon = new St.Icon({
        icon_name: MOVE_TO_PREVIOUS_WORKSPACE_ICON_NAME,
        style_class: "system-status-icon",
      });
      this.window_control_button_previous.set_child(
        this.window_control_button_previous_icon
      );
      this.window_control_button_previous.connect(
        "button-press-event",
        this._move_to_previous_workspace.bind(this)
      );

      this.window_control_button_next = new St.Bin({
        visible: true,
        reactive: true,
        can_focus: true,
        track_hover: true,
      });
      this.window_control_button_next_icon = new St.Icon({
        icon_name: MOVE_TO_NEXT_WORKSPACE_ICON_NAME,
        style_class: "system-status-icon",
      });
      this.window_control_button_next.set_child(
        this.window_control_button_next_icon
      );
      this.window_control_button_next.connect(
        "button-press-event",
        this._move_to_next_workspace.bind(this)
      );
    ...
    _move_to_next_workspace() {
      ...
      if (this.focused_window && this.active_ws_index < this.ws_count - 1) {
        this.focused_window.change_workspace_by_index(
          this.active_ws_index + 1,
          false
        );
...
var WorkspacesBar = GObject.registerClass(
  class WorkspacesBar extends PanelMenu.Button {
    _init() {
      ...
      this.workspaces_settings = new Gio.Settings({
      ...
      this.window_tooltip.label = new St.Label({
      ...
      for (let ws_index = 0; ws_index < this.ws_count; ++ws_index) {
        ...
        if (FAVORITES_FIRST) {
          ...
          this.ws_current.windows = this.ws_current
            .list_windows()
            .sort(this._sort_windows_favorites_first.bind(this));
        } else {
          this.ws_current.windows = this.ws_current
            .list_windows()
        ...
        for (
          ...
          if (
            this.window &&
            this.window_type_whitelist.includes(this.window.get_window_type())
          ) {
    ...
    _create_window_button(ws_index, w) {
      ...
      var move_previous = new St.Bin({
        visible: false,
        reactive: true,
        can_focus: true,
        track_hover: true,
      });
      var move_previous_icon = new St.Icon({
        icon_name: MOVE_TO_PREVIOUS_WORKSPACE_ICON_NAME,
        style_class: "system-status-icon",
      });
      move_previous.set_child(move_previous_icon);

      var move_next = new St.Bin({
        visible: false,
        reactive: true,
        can_focus: true,
        track_hover: true,
      });
      var move_next_icon = new St.Icon({
        icon_name: MOVE_TO_NEXT_WORKSPACE_ICON_NAME,
        style_class: "system-status-icon",
      });
      ...
      if (!w.is_on_all_workspaces()) {
        move_previous.connect("button-press-event", () =>
          this._move_to_previous_workspace(ws_index, w)
        );
        move_next.connect("button-press-event", () =>
          this._move_to_next_workspace(ws_index, w)
        );
      }
      ...
      if (!w.is_on_all_workspaces() || ws_index == 0) {
        // create button
        var w_box = new St.BoxLayout({
          visible: true,
          reactive: true,
          can_focus: true,
          track_hover: true,
          style_class: "window-box",
        });
        ...
        if (
          !w_box_icon ||
          w_box_icon.get_style_class_name() == "fallback-app-icon"
        ) {
          w_box_icon = new St.Icon({
            icon_name: FALLBACK_ICON_NAME,
            icon_size: ICON_SIZE,
          });
        }
        w_box_button.set_child(w_box_icon);
        w_box_button.connect("button-press-event", (widget, event) =>
    ...
    _sort_windows_favorites_first(w1, w2) {
      ...
      this.w1_is_favorite = AppFavorites.getAppFavorites().isFavorite(
        this.w1_app.get_id()
      );
      this.w2_is_favorite = AppFavorites.getAppFavorites().isFavorite(
        this.w2_app.get_id()
      );
      ...
      if (this.w1_is_favorite && this.w2_is_favorite) {
        if (this.w1_app == this.w2_app) {
          ...
          return (
            this.favorites_list.indexOf(this.w1_app) -
            this.favorites_list.indexOf(this.w2_app)
          );
        }
      }
      if (this.w1_is_favorite && !this.w2_is_favorite) {
    ...
    _on_button_hover(w_box, window_title) {
      if (window_title && w_box && w_box.get_hover()) {
        this.window_tooltip.set_position(
          w_box.get_transformed_position()[0],
        ...
        this.hide_tooltip_timeout = GLib.timeout_add_seconds(
...
class Extension {
  ...
  _get_settings() {
    this.settings = ExtensionUtils.getSettings(
      "org.gnome.shell.extensions.babar"
    );

    this.settings_already_changed = false;
    this.settings_changed = this.settings.connect(
      "changed",
      this._settings_changed.bind(this)
    );
    ...
    ROUNDED_WORKSPACES_BUTTONS = this.settings.get_boolean(
      "rounded-workspaces-buttons"
    );
    TOOLTIP_VERTICAL_PADDING = this.settings.get_int(
    ...
    DISPLAY_WINDOW_CONTROL = this.settings.get_boolean(
    ...
    DISPLAY_WORKSPACES_THUMBNAILS = this.settings.get_boolean(
      "display-workspaces-thumbnails"
    );
  }
  ...
  enable() {
    ...
    if (DISPLAY_PLACES_ICON) {
      ...
      this.extensions_changed = Main.extensionManager.connect(
    ...
    if (!DISPLAY_WORKSPACES_THUMBNAILS) {
      this.showing_overview = Main.overview.connect(
        "showing",
        this._hide_workspaces_thumbnails.bind(this)
      );
    }
  }
